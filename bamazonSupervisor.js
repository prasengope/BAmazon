		...
		.prompt([
			{
				name: 'department',
				message:
					'Please type in the name of the department you would like to add. Duplicate departments will not be accepted',
				type: 'input'
			},
			{
				name: 'overhead',
		...
		.then(function(answer) {
			...
			connection.query('SELECT * FROM departments', function(err, res) {
				...
				if (arr.indexOf(answer.department.toLowerCase()) !== -1) {
					...
					connection.query(
						`INSERT INTO departments (department_name, over_head_costs) VALUES ("${answer.department}", ${parseFloat(
							answer.overhead
						)})`,
						function(err, res) {
							if (err) throw err;
							console.log(
								'Your department: ' +
									answer.department +
									' has been entered into the database with an overhead cost value of ' +
									answer.overhead +
									'.'
							);
							menu();
