...
function addInv() {
	connection.query('SELECT * FROM products', function(err, res) {
		...
		inquirer
			...
			.then(function(answer) {
				connection.query(
					`UPDATE products SET stock_quantity=stock_quantity + ${answer.quantity} WHERE item_id=${answer.item_selection}`,
					function(err, res) {
		...
		inquirer
			.prompt([
				...
				},
				{
					name: 'department',
					message:
						'Which department does this item belong in? If the department you wish to add does not exist, please create that department using the Supervisor program.',
					type: 'list',
