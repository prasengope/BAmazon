			...
			.then(function(answer) {
				...
				connection.query('SELECT * FROM products', function(err, res) {
					...
					if (selection.stock_quantity - quantity >= 0) {
						...
						console.log(
							'Sorry, but we have insufficient stock to fulfill your order. You requested ' +
								quantity +
								' of this item, but we only have ' +
								selection.stock_quantity +
								' in stock. Returning to the item list now.'
						);
						// Back to menu
						startMenu();
					}
...
function buyItem(item, quantity) {
	connection.query(
		`UPDATE products SET stock_quantity=(${parseFloat(item.stock_quantity)} - ${parseFloat(
			quantity
		)}), product_sales=(product_sales + (${parseFloat(item.price)} * ${parseFloat(
			quantity
		...
		function(err, res) {
			...
			connection.query(`SELECT department_name FROM products WHERE item_id=${item.item_id}`, function(err, res) {
				...
				connection.query(
					`SELECT product_sales FROM departments WHERE department_name="${department}"`,
					function(err, res) {
						let totalSale = res[0].product_sales;
						totalSale = parseFloat(totalSale) + parseFloat(sales);
						connection.query(
							`UPDATE departments SET product_sales=${totalSale} WHERE department_name="${department}"`,
							function(err, res) {
								if (err) throw err;
